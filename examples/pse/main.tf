...
# without booting a VM. This is what lets the Service Edge's Key Vault grant be
# in place and propagated BEFORE the VM boots and writes its OAuth2 user code --
# the Azure analog of attaching an AWS IAM instance profile at launch. A single
# shared identity is attached to every Service Edge VM in the deployment.
...
################################################################################
# 7. Create Network Security Group(s) for the Service Edge interface(s)
################################################################################
module "pse_nsg" {
...
################################################################################
# 9. (OAuth2 flow only) Create the ZPA Service Edge Group with OAuth2 user
