# Zscaler "pse_vmss" deployment type

This deployment type is intended for brownfield/production purposes. By default, it will create a new Resource Group; 1 VNet; at least 1 Private Service Edge private subnet; at least 1 NAT Gateway with Public IP Address association to the Private Service Edge subnets; at least 1 Virtual Machine Scale Set (VMSS); and generates a local key pair .pem file for ssh access.<br>
...
Additionally: Depending on the configuration, creates 1 or more Flexible Orchestration Virtual Machine Scale Sets (VMSS) and scaling policies for Private Service Edges in private subnet(s).
...
### Option 1 (guided):
Optional: Edit the terraform.tfvars file under your desired deployment type (ie: pse_vmss) to setup your Service Edge Group (Details are documented inside the file)
...
### Option 2 (manual):
Modify/populate any required variable input values in pse_vmss/terraform.tfvars file and save.

From pse_vmss directory execute:
- terraform init
- terraform apply
