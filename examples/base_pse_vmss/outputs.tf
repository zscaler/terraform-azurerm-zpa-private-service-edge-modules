...
2) From the bastion, SSH to a Private Service Edge scale-set instance (replace <PSE-PRIVATE-IP>)
ssh -i ${var.name_prefix}-key-${random_string.suffix.result}.pem zsroot@<PSE-PRIVATE-IP> -o "proxycommand ssh -W %h:%p -i ${var.name_prefix}-key-${random_string.suffix.result}.pem ubuntu@${module.bastion.public_ip}"
