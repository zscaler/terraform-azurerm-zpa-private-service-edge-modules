#!/bin/bash
################################################################################
# Zscaler Private Service Edge User Data Script (Zscaler Marketplace Image - Azure)
...
#   - provisioning_key : write the ZPA provisioning key to the Service Edge and
#                        let it self-enroll. No Key Vault interaction required.
#
...
# The Zscaler Private Service Edge appliance is a hardened EL9 image that does NOT ship
...
systemctl stop zpa-service-edge 2>/dev/null || true

# Write the provisioning key created via the ZPA provider to the Service Edge.
# Keep the key between double quotes.
echo "${provisioning_key}" > /opt/zscaler/var/service-edge/provision_key
...
# Ensure zpa-service-edge service is running (generates OAuth token in /etc/issue)
systemctl start zpa-service-edge 2>/dev/null || true

################################################################################
# Install the Azure CLI (non-interactively).
#
# The hardened Private Service Edge appliance does NOT ship the Azure CLI, and cloud-init
