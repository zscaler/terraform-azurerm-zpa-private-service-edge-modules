#!/usr/bin/bash
################################################################################
# RHEL9 Private Service Edge User Data Script (Azure)
...
# Wait for the Private Service Edge to download the latest build, then restart
sleep 60
systemctl stop zpa-service-edge
systemctl start zpa-service-edge
...
# Start zpa-service-edge service to generate the OAuth token in /etc/issue
systemctl start zpa-service-edge
