...
| <a name="input_user_codes"></a> [user\_codes](#input\_user\_codes) | OAuth2 user codes retrieved from deployed Private Service Edge VMs (/etc/issue). These codes are required for enrolling Private Service Edges using the new OAuth2 authentication method. | `list(string)` | `[]` | no |
