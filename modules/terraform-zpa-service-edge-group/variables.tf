...
variable "pse_group_city_country" {
  type        = string
  description = "Optional: City and country of this Private Service Edge Group. example 'San Jose, US'"
  default     = ""
}
...
variable "user_codes" {
  type        = list(string)
  description = "OAuth2 user codes retrieved from deployed Private Service Edge VMs (/etc/issue). These codes are required for enrolling Private Service Edges using the new OAuth2 authentication method."
  default     = []
}
