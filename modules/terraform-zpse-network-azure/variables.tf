...
variable "base_only" {
  type        = bool
  description = "Default is false. Only applicable for base deployment type resulting in workload and bastion hosts, but no Private Service Edge resources. Setting this to true will point workload route able to Internet"
  default     = false
}
...
variable "byo_vnet" {
  type        = bool
  description = "Bring your own Azure VNet for Private Service Edge. If false, a new VNet will be created automatically"
  default     = false
}
...
variable "byo_subnets" {
  type        = bool
  description = "Bring your own Azure subnets for Private Service Edge. If false, new subnet(s) will be created automatically. Default 1 subnet for Private Service Edge if 1 or no zones specified. Otherwise, number of subnes created will equal number of Private Service Edge zones"
  default     = false
}
...
variable "existing_nat_gw_subnet_association" {
  type        = bool
  description = "Set this to true only if both byo_nat_gws and byo_subnets variables are true. this implies that there are already NAT Gateway resources associated to subnets where Private Service Edges are being deployed to"
  default     = false
}
