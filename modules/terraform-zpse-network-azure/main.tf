...
################################################################################
# Private (Private Service Edge) Subnets
################################################################################
# Create Private Service Edge Subnet
resource "azurerm_subnet" "pse_subnet" {
  ...
  address_prefixes     = var.pse_subnets != null ? [element(var.pse_subnets, count.index)] : [cidrsubnet(var.network_address_space, 8, count.index + 200)]
}

# Or reference an existing subnet
data "azurerm_subnet" "pse_subnet_selected" {
  count                = var.byo_subnets == false ? length(azurerm_subnet.pse_subnet[*].id) : length(var.byo_subnet_names)
  name                 = var.byo_subnets == false ? azurerm_subnet.pse_subnet[count.index].name : element(var.byo_subnet_names, count.index)
  resource_group_name  = var.byo_vnet == false ? data.azurerm_virtual_network.vnet_selected.resource_group_name : var.byo_vnet_subnets_rg_name
  virtual_network_name = var.byo_vnet == false ? data.azurerm_virtual_network.vnet_selected.name : var.byo_vnet_name
}

# Associate Cloud Connector Subnet to NAT Gateway
resource "azurerm_subnet_nat_gateway_association" "pse_subnet_nat_association" {
  count          = var.existing_nat_gw_subnet_association == false ? length(data.azurerm_subnet.pse_subnet_selected[*].id) : 0
  subnet_id      = data.azurerm_subnet.pse_subnet_selected[count.index].id
  nat_gateway_id = data.azurerm_nat_gateway.ngw_selected[count.index].id
